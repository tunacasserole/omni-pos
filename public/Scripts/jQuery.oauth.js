(function (window, document, $, undefined) {
    ...
    $.oauth = function (options) {
        var d = $.Deferred();
        $.when(
            require('str_sha1', '../Scripts/sha1.js'),
            require('OAuth', '../Scripts/oauth.js')
            ).then(function () {
    ...
    function addOAuthStuffs(options) {
        ...
        if (options.url.indexOf(':') == -1) {
            if (options.url.substr(0, 1) == '/') { options.url = location.protocol + '//' + location.host + options.url; }
            else { options.url = location.href.substr(0, location.href.lastIndexOf('/') + 1) + options.url; }
        }
        var message = {
        ...
        OAuth.SignatureMethod.sign(message, {
            consumerSecret: options.consumerSecret, tokenSecret: options.tokenSecret
        });
